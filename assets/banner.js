// Lightweight accessible carousel (robust init)
(function(){
  function initBanner(){
    ...
    // Ensure slides use exact widths (in case of responsive/layout changes)
    function setSlideSizes(){
      // each slide already uses flex: 0 0 100% in CSS; this ensures no sub-pixel drift
      // but we keep this placeholder if we later want to compute pixel widths
    }
    ...
    // keep layout robust on resize
    window.addEventListener('resize', () => {
      setSlideSizes();
      // ensure transform remains correct after resize
      goTo(current);
    });
  }
